 ...
 * Tile компрессора.
 */
public class TileCompressor extends BaseTileMachine implements IEnergySink {
    public int progress;

    public TileCompressor() { super(24000, 2); }

    @Override
    public void updateEntity() {
        if (!worldObj.isRemote) {
            boolean working = canProcess() && energy >= 18;
            if (working) {
                progress++;
                energy -= 18;
                if (progress >= 100) {
    ...
    private boolean canProcess() {
        ...
        ItemStack result = CompressorRecipes.getResult(inventory[0]);
    ...
    private void processItem() {
        ...
        ItemStack result = CompressorRecipes.getResult(inventory[0]).copy();
