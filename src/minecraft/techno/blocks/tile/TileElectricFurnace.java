...
public class TileElectricFurnace extends BaseTileMachine implements IEnergySink {
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            boolean working = canSmelt() && energy >= 16;
            if (working) {
                progress++;
                energy -= 16;
                if (progress >= 100) {
    ...
    public void validate() {
        super.validate();
        if (worldObj != null && !worldObj.isRemote) GlobalEnergyNetworkRegistry.get(worldObj.provider.dimensionId).addSink(this);
    }

    @Override
    public void invalidate() {
        if (worldObj != null && !worldObj.isRemote) GlobalEnergyNetworkRegistry.get(worldObj.provider.dimensionId).removeSink(this);
        super.invalidate();
    }
